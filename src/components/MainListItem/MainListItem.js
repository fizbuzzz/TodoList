...
class MainListItem extends Component {
    ...
    render() {
        const { text , deleteItem, onChangeChecked, spanClasses, checboxClasses} = this.props;
        return (
            <li className="main__item">
                {/* <input   type="checkbox" name="" id="" className="main__item-checkbox" 
                onClick={onChangeChecked}/> */}
                <div onClick={onChangeChecked} className={checboxClasses}>
                    
                </div>
                <span className={spanClasses}>{text}</span>
                <div className="main__item-trash"
                onClick={deleteItem}>
                    <svg
                        viewBox="0 0 18 18"
                        fill="none"
